...
class CustomDrawerLayout : DrawerLayout {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        val widthMeasureSpec: Int = MeasureSpec.makeMeasureSpec(MeasureSpec.getSize(widthMeasureSpec), MeasureSpec.EXACTLY)
        val heightMeasureSpec: Int = MeasureSpec.makeMeasureSpec(MeasureSpec.getSize(heightMeasureSpec), MeasureSpec.EXACTLY)

        super.onMeasure(widthMeasureSpec, heightMeasureSpec)
    }

    companion object {
        const val MI_TYPE_SLIDE_WITH_CONTENT = 1 // For sliding view with content
        const val MI_TYPE_DOOR_IN = 2 // Open a drawer inside with content sliding with scale and rotation both.
        const val MI_TYPE_DOOR_OUT = 3 // Open a drawer outside with content sliding with scale and rotation both.
        const val MI_TYPE_SLIDE = 4 // Open a drawer with content sliding with scale only
    }
    ...
    internal var drawerListener: DrawerListener? = null
    ...
    constructor(context: Context, attrs: AttributeSet?) : this(context, attrs, 0) {
        val arr = context.obtainStyledAttributes(attrs, R.styleable.DrawerLayout)
        val sliderType = arr.getInt(R.styleable.DrawerLayout_sliderType, 0)
    ...
    private fun setDrawerView(slideType: Int) {
